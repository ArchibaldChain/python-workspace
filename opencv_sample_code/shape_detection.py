...
class ShapeAnalysis:
    ...
    def analysis(self, frame):
        ...
        out_binary, contours, hierarchy = cv.findContours(
        ...
        for cnt in range(len(contours)):
            ...
            print("周长: %.3f, 面积: %.3f 颜色: %s 形状: %s " %
                  (p, area, color_str, shape_type))
    ...
    def draw_text_info(self, image):
        ...
        cv.putText(image, "triangle: "+str(c1), (10, 20),
                   cv.FONT_HERSHEY_PLAIN, 1.2, (255, 0, 0), 1)
        cv.putText(image, "rectangle: " + str(c2), (10, 40),
                   cv.FONT_HERSHEY_PLAIN, 1.2, (255, 0, 0), 1)
        cv.putText(image, "polygons: " + str(c3), (10, 60),
                   cv.FONT_HERSHEY_PLAIN, 1.2, (255, 0, 0), 1)
        cv.putText(image, "circles: " + str(c4), (10, 80),
                   cv.FONT_HERSHEY_PLAIN, 1.2, (255, 0, 0), 1)
        return image
...
if __name__ == "__main__":
    src = cv.imread(
        "I:\Programming\Cpp_workspace\CppLearningCode\CppAssignment\opencv_test\Lab_10\line-detection.jpg")
